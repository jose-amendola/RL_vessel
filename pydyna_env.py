...
class PyDynaEnv(Env):
    ...
    def calculate_reward(self, obs):
            rew = -np.abs(obs[1] + 166.6)**2 - 1000*np.abs(obs[2])**2 - np.abs(obs[3])**2 + 5000/obs[4]
            return rew
    ...
    def reset(self):
        original_state = self.simulator.get_vessel_pos_vel()
        init_pos = [11000, 5280]
        theta = np.deg2rad(90-(-103.4))
        init_vel = 3
        init = [init_pos[0], init_pos[1], original_state[2], original_state[3], original_state[4], theta,
                init_vel*np.cos(theta), init_vel*np.sin(theta), original_state[8],
                original_state[9], original_state[10], original_state[11]]
