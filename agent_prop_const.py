...
class QLearning():
    def __init__(self, *args, **kwargs):
        ...
        self.actions = [-0.8,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.8]
                        #[-0.6,-0.4,-0.2,0,0.2,0.4,0.6]]
        self.discount = kwargs.get("discount", 0.9)
        self.learning_rate = kwargs.get("learning_rate", 0.1)
        self.last_action = [0]
        self.Q = np.zeros(len(self.actions))
        self.W = np.zeros((len(self.actions), self.s.shape[0], self.s.shape[1], self.s.shape[2], self.s.shape[3]))
    ...
    def update(self, next_state, reward, step):
        ...
        Q = np.sum(self.W*phi,axis=(1,2,3,4))
    ...
    def act(self, state=None, train = True):
        ...
        if not train:
            ...
            self.Q = np.sum(self.W*phi,axis=(1,2,3,4))
            epsilon = 1
...
#            sys.stdout.flush()
            ...
            if negative_side:
                action = -action
            observation, reward, done, info = self.ship.step([action,0.3])
            negative_side = False
            if done:
                break
            if observation[0]<0:
...
if __name__ == "__main__":
    ...
    for i in range(1,7):
        ...
        agent.setEps(i*0.1)
        np.save('w_20181015_eps'+str(i-1)+'_iter'+str(100*i)+'_r2_propcte.np',agent.getW())
